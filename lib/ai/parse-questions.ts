...
function fixTruncatedJSON(text: string, metrics: JSONParseMetrics): string {
  ...
  if (afterLastBrace.length > 0) {
    console.warn('[parse-questions] Detected truncated content, trimming to last closing brace')
  ...
  // Count braces to find complete JSON structure
  let braceCount = 0
  let bracketCount = 0
  let inString = false
  ...
  if ((inString || braceCount !== 0 || bracketCount !== 0) && lastCompletePos > 0) {
    console.warn('[parse-questions] Detected incomplete JSON structure, truncating to last complete position')
    metrics.wasTruncated = true
    metrics.truncationRecovered = true
...
export function parseAnalysis(rawText: string): HabitParseResult {
  ...
  try {
    ...
    console.error('[parse-questions] Truncation fix failed:', truncError)
    metrics.wasTruncated = true
  ...
  try {
    ...
    console.log('[parse-questions] JSON Robustness:', {
    ...
    console.error('[parse-questions] Failed to parse habit analysis JSON')
